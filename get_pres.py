...
class NameForm(FlaskForm):
    nameMSICat = SelectField('MSI Categorization', choices=[('MSI-high', 'MSI-high'), ('MSI-ambiguous', 'MSI-ambiguous'), ('MSI-low', 'MSI-low')])
...
def MSIanalyze():
    ...
    if form.validate_on_submit():
        if request.method == 'POST':
            if form.nameMSICat.data == 'MSI-high':
                return redirect('/analyzeresults')
                print('hello world')
            elif(form.nameMSICat.data == 'MSI-ambiguous'):
                return redirect('/analyzeresultsambiguous')
                print('hello world')
            elif(form.nameMSICat.data == 'MSI-low'):
                return redirect('/analyzeresultslow')
                print('hello world')
    return render_template('analyze.html', form=form)

@app.route('/analyzeresults')
def Display_IMG():
    Flask_Logo = os.path.join(app.config['UPLOAD_FOLDER'], 'flask-logo.png')
    return render_template("analyzeresults.html", user_image=Flask_Logo)

@app.route('/analyzeresultsambiguous')
def Display_IMG2():
    Flask_Logo = os.path.join(app.config['UPLOAD_FOLDER'], 'flask-logo2.png')
    return render_template("analyzeresultsambiguous.html", user_image=Flask_Logo)

@app.route('/analyzeresultslow')
def Display_IMG3():
    Flask_Logo = os.path.join(app.config['UPLOAD_FOLDER'], 'flask-logo3.png')
    return render_template("analyzeresultslow.html", user_image=Flask_Logo)
