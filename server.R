...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$Genesearch, {
    qry <- paste("SELECT gene_name, msi_val, upperalignment, loweralignment, sample, mononucseq FROM msi_alignment_scores WHERE gene_name LIKE", paste0("'", input$name,"%'"), "LIMIT", paste0(input$var2, ";"))
  ...
  observeEvent(input$Analyzedata, {
    ...
    qry1 <- paste("SELECT * FROM msi_alignment_scores WHERE gene_name LIKE", paste0("'", input$dataname,"%';"))
    ...
    qry4 <- paste("SELECT sample, count(sample) FROM msi_alignment_scores WHERE gene_name =", paste0("'", input$dataname,"' GROUP BY sample;"))
    print(qry4)
    ...
    output$output1 <- renderUI({
      h3(paste('Data Analysis for', input$var, 'Oncogene', input$dataname, 'List'))
    })
